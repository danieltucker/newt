 ...
 *
 * The query and — far more importantly — the subscription scoping both live in
 * lib/articleSearch now, because the search page asks the same question through
 * its own endpoint. Two copies of that scoping rule is how one of them ends up
 * searching the whole instance.
 */
router.get('/search', searchLimiter, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    const articles = await searchArticles(req.userId!, raw, { tagged, limit });
    res.json({ articles });
  } catch (err) {
